...
def make_blink_schedule(n_frames, avg_interval=80, blink_len=4):
    """
    Returns a set of frame indices where the avatar should be blinking.
    - avg_interval: approx frames between blinks (e.g. 80 ≃ every 3s at 25fps)
    - blink_len: how many consecutive frames each blink lasts
    """
    schedule = set()
    t = 0
    while t<n_frames:
        t += random.randint(avg_interval - 20, avg_interval + 20)
        for b in range(blink_len):
            schedule.add(min(t + b, n_frames-1))
    return schedule
